...
interface MembersClientPageProps {
  ...
  initialGardenOwner: string | null;
...
export default function MembersClientPage({
  ...
  const [gardenOwner] = useState<string | null>(initialGardenOwner);
  ...
  const handleAdd = async (userId: number) => {
    try {
      ...
      if (res.ok) {
        const addedUser = results.find(u => u.id === userId);
        setMembers(prev => {
          if (!addedUser || prev.some(member => member.id === userId)) {
            return prev;
          }
